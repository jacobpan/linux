...
+----------------------+----------------------+----------------+----------------+
| direct_hwpt_id       | IOMMU_HWPT_ALLOC     | IOMMUFD object | Kernel-managed |
|                      | with HWPT_DIRECT     | ID namespace   | attachable     |
|                      |                      | in one ctx     | direct HWPT /  |
|                      |                      |                | domain object. |
+----------------------+----------------------+----------------+----------------+
```
...
direct attach/detach hypercalls. The existing `IOMMU_VDEVICE_ALLOC` UAPI
has the right parent and identity model. MSHV direct attach uses this
...
VMM                         IOMMUFD / IOMMU Driver          Hypervisor
 ...
 | VFIO_DEVICE_BIND_IOMMUFD          |                           |
 |---------------------------------->|                           |
 | IOMMU_VIOMMU_ALLOC (DIRECT)       | viommu_init()             |
 ...
 |---------------------------------->|                           |
 | IOMMU_HWPT_ALLOC (DIRECT)         | alloc_domain_direct()     |
 |  pt_id = viommu_id                |  create HWPT_DIRECT       |
 |  data_type = DIRECT               |  no IOAS backing          |
 |---------------------------------->|                           |
 | VFIO_DEVICE_ATTACH_IOMMUFD_PT     | attach_dev()              |
 |  pt_id = hwpt_direct_id           |--ATTACH_DEVICE----------->|
 |---------------------------------->|                           |
 | VFIO_DEVICE_DETACH_IOMMUFD_PT     | detach_dev()              |
 |  pt_id = hwpt_direct_id           |--DETACH_DEVICE----------->|
 |---------------------------------->|                           |
```

Unlike domain attach, this flow does not allocate an IOAS or paging HWPT, and
does not call `IOMMU_IOAS_MAP` or `IOMMU_IOAS_UNMAP`. The hypervisor direct
attach operation happens at device attach time using the VM identity from the
...
`virt_id` field carries the direct-attach VM device ID chosen by the VMM.
...
* The direct attach implementation uses one VM-visible device identifier
  ...
  environments: the direct-attach VM device ID. If a baremetal root
...
* Treat `iommu_vdevice_alloc::virt_id` as the direct-attach VM device ID.
...
* Implement MSHV externally managed domain allocation for direct HWPTs under
  `IOMMU_HWPT_DATA_DIRECT`.
* Implement driver-private state for the direct-attach VM device ID if needed by
  the vDEVICE.
* Wire `attach_dev`/detach to the direct attach/detach hypercalls
  on the direct HWPT, using the VM identity held by the vIOMMU and the VM
