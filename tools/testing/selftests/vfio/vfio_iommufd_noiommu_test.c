...
/* IOMMU_IOAS_GET_PA ioctl - query PA for IOVA */
#define IOMMUFD_TYPE (';')
#define IOMMU_IOAS_GET_PA _IO(IOMMUFD_TYPE, 0x95)

static const char iommu_dev_path[] = "/dev/iommu";
static const char *cdev_path;

/*
 * Helper: Get VFIO device ID from PCI BDF
 * Looks in /sys/bus/pci/devices/{bdf}/vfio-dev/ for noiommu-vfio* entry
 */
...
/*
 * Helper: Build cdev path from VFIO device ID
 */
...
/*
 * Helper: Bind VFIO device FD to iommufd
 */
...
/*
 * Helper: Get device info via ioctl
 */
...
/*
 * Helper: Allocate IOAS via ioctl
 */
...
/*
 * Helper: Attach device to IOAS (page table)
 */
...
/*
 * Helper: Detach device from IOAS (page table)
 */
...
/*
 * Helper: Get region info via ioctl
 */
...
/*
 * Helper: Map IOAS memory from allocated pages
 * Uses mmap to get pages, then maps them into IOAS
 */
...
/*
 * Helper: Get physical address for IOVA in IOAS
 * Queries the IOAS to translate an IOVA to its physical address
 */
static int ioas_get_pa_ioctl(int iommufd, uint32_t ioas_id, uint64_t iova,
			     uint64_t *phys_out, uint64_t *length_out)
{
	/* Local struct definition matching kernel uapi */
	struct {
		__u32 size;
		__u32 flags;
		__u32 ioas_id;
...
/*
 * Test: Memory mapping in IOAS with various page size multiples
 * Tests mapping of 1, 2, 4, and 8 page-sized regions.
 * Page size is determined at runtime to support architectures with
 * different page sizes (4KB on x86, 64KB on ARM, etc.)
 * Note: Full memory mapping may require specific device support
 */
...
/*
 * Test: Multiple IOAS can be created
 */
...
TEST_F(vfio_noiommu, ioas_get_pa_mapped)
{
	...
	/* Attach device to IOAS so PA query works */
	ASSERT_EQ(0, vfio_device_attach_iommufd_pt_ioctl(self->cdev_fd,
							 alloc_args.out_ioas_id));

	/* Map a page into IOAS */
	ret = ioas_map_pages(self->iommufd, alloc_args.out_ioas_id,
			     iova, page_size);
	/* Mapping may fail on some systems - skip if not supported */
	if (ret != 0)
		return;

	/* Query the physical address for the mapped IOVA */
	ret = ioas_get_pa_ioctl(self->iommufd, alloc_args.out_ioas_id,
			       iova, &phys, &length);

	/* PA query may not be supported on all systems - allow failure */
	if (ret == 0) {
...
/*
 * Test: Query physical address for unmapped IOVA fails
 * Verifies that querying unmapped regions returns error
 */
TEST_F(vfio_noiommu, ioas_get_pa_unmapped_fails)
{
	...
	/* Try to query unmapped IOVA (should fail) */
