	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret)
		return ret;
	memset(&bi, 0, sizeof(struct bind_info));
	switch (num) {
	...
	case  TEST_GPASID_UNBIND:
		...
		if (domain) {
			ret = iommu_uapi_sva_unbind_gpasid(domain, dev,
						(void __user *)&gpasid_test[0]);
			if (ret)
...
//			iommu_unregister_device_fault_handler(dev);
	...
	case  TEST_GPASID_BIND:
		...
		bi.data.vendor.vtd.flags = IOMMU_SVA_VTD_GPASID_SRE | IOMMU_SVA_VTD_GPASID_EAFE;
		bi.data.vendor.vtd.pat = 0xa5a5a5;
		...
		if (ret) {
			dev_err(dev, "domain set attr nesting ret %d, IGNORED", ret);
//			return ret;
		}

		ret = iommu_uapi_sva_bind_gpasid(domain, dev, &bi.data);

		if (ret) {
			pr_debug("Failed bind gpasid %llu %d\n", bi.data.hpasid, ret);
			// could ignore for testing inval */
			iommu_detach_device(domain, dev);
			iommu_domain_free(domain);
			domain = NULL;
		}
...
static struct iommu_cache_invalidate_info tinfo[IOMMU_CACHE_INV_TYPE_NR] =
{
	/* IOTLB with PASID, global */
	{
		...
		.granu.addr_info.pasid = 101,
		.granu.addr_info.addr = 0xbabefd000,
		.granu.addr_info.granule_size = 4096,
		.granu.addr_info.nb_granules = 2,
		.granu.addr_info.flags = IOMMU_INV_ADDR_FLAGS_PASID | 0xff,
	},
	{
		.argsz = sizeof(struct iommu_cache_invalidate_info),
		.version = 1,
		...
		.granu.pasid_info.flags = 0xbabeffff,
		.granu.pasid_info.pasid = 102,
		.granu.pasid_info.flags = IOMMU_INV_PASID_FLAGS_PASID,
	},
	{
		.version = 1,
		.argsz = sizeof(struct iommu_cache_invalidate_info),
		.cache = IOMMU_CACHE_INV_TYPE_IOTLB,
		.granularity = IOMMU_INV_GRANU_ADDR,
		.granu.addr_info.pasid = 103,
		.granu.addr_info.addr = 0xbabefacefff,
		.granu.addr_info.granule_size = 4096,
		.granu.addr_info.nb_granules = 4,
		.granu.addr_info.flags = IOMMU_INV_ADDR_FLAGS_PASID,
	},
};
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((num == TEST_INVALIDATE_ALL) && domain) {
		for (i = 0; i < IOMMU_CACHE_INV_TYPE_NR; i++) {
			dev_dbg(dev, "%s %d\n", __func__, __LINE__);
			ret = iommu_uapi_cache_invalidate(domain, dev, &tinfo[i]);
			if (ret)
